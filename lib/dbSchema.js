...
//   products/{productId}          — has optional categoryId
//   shoppingLists/{listId}
//     items/{itemId}              — snapshots categoryName + categoryIconUrl
//
...
export const OrganizationFactory = {
  ...
  create: async ({ name, createdBy, isPrivate = false }) => {
  ...
  /** Get all non-private organizations (for access request search) */
  getAllNonPrivate: async () => {
    const snap = await getDocs(collection(db, 'organizations'));
    return { docs: snap.docs.filter((d) => d.data().isPrivate !== true) };
  },
...
export const GroupFactory = {
  ...
  /** Create a new group */
  create: async (orgId, { name, memberIds = [], imageUrl = null }) => {
...
export const CategoryFactory = {
  collection: (orgId) => collection(db, 'organizations', orgId, 'categories'),

  doc: (orgId, categoryId) =>
    doc(db, 'organizations', orgId, 'categories', categoryId),

  create: async (orgId, { name, iconUrl = '', arasaacId = null, color = '#4CAF50', centralCategoryId = null, createdBy }) => {
    const data = { name, iconUrl, arasaacId, color, createdBy, createdAt: serverTimestamp() };
    if (centralCategoryId) data.centralCategoryId = centralCategoryId;
    return addDoc(collection(db, 'organizations', orgId, 'categories'), data);
  },
  ...
  getById: (orgId, categoryId) =>
    getDoc(doc(db, 'organizations', orgId, 'categories', categoryId)),

  update: (orgId, categoryId, data) =>
    updateDoc(doc(db, 'organizations', orgId, 'categories', categoryId), data),

  delete: (orgId, categoryId) =>
    deleteDoc(doc(db, 'organizations', orgId, 'categories', categoryId)),
};

// ---------------------------------------------------------------------------
// TagFactory — product tags binnen een organisatie
// ---------------------------------------------------------------------------
export const TagFactory = {
  collection: (orgId) => collection(db, 'organizations', orgId, 'tags'),

  doc: (orgId, tagId) => doc(db, 'organizations', orgId, 'tags', tagId),

  create: async (orgId, { name, imageUrl = null, createdBy }) =>
    addDoc(collection(db, 'organizations', orgId, 'tags'), {
      name,
      imageUrl,
      createdBy,
      createdAt: serverTimestamp(),
  ...
  getById: (orgId, tagId) =>
    getDoc(doc(db, 'organizations', orgId, 'tags', tagId)),

  update: (orgId, tagId, data) =>
    updateDoc(doc(db, 'organizations', orgId, 'tags', tagId), data),

  delete: (orgId, tagId) =>
    deleteDoc(doc(db, 'organizations', orgId, 'tags', tagId)),
};
...
export const ProductFactory = {
  ...
  create: async (orgId, { name, imageUrl, unit = 'stuks', categoryId = null, tagIds = [], centralProductId = null, createdBy }) => {
    const data = {
      name,
      imageUrl,
      unit,
    ...
    if (centralProductId) data.centralProductId = centralProductId;
    return addDoc(collection(db, 'organizations', orgId, 'products'), data);
  },
...
export const ListItemFactory = {
   ...
   * Snapshots the product name, image, category and tags at the time of creation,
   * so the list remains correct even if the product is later edited.
   */
  create: async (orgId, listId, { productId, productName, productImageUrl, categoryId = null, categoryName = null, categoryIconUrl = null, tags = [], quantity, order }) => {
    return addDoc(
      collection(db, 'organizations', orgId, 'shoppingLists', listId, 'items'),
      {
        productId,
        ...
        categoryIconUrl,    // snapshot
        tags,               // snapshot: [{ tagId, tagName, tagImageUrl }]
        quantity,
        checked: false,
        order,
        createdAt: serverTimestamp(),
  ...
  /**
   * Subscribe to real-time item updates for a shopping list.
   * Calls onUpdate with the full items array on every change.
   * Returns an unsubscribe function.
   */
  subscribe: (orgId, listId, onUpdate) =>
    onSnapshot(
      query(
        collection(db, 'organizations', orgId, 'shoppingLists', listId, 'items'),
        orderBy('order', 'asc')
...
export const StorageFactory = {
   ...
   * Returns the public download URL.
   */
  uploadCategoryIcon: async (orgId, categoryId, file) => {
    const extension = file.name.split('.').pop();
    const storageRef = ref(
      storage,
      `organizations/${orgId}/categories/${categoryId}/${Date.now()}.${extension}`
    );
    const snapshot = await uploadBytes(storageRef, file);
    return getDownloadURL(snapshot.ref);
  },
  ...
  /**
   * Upload a central category icon (admin-created, not org-scoped).
   * Returns the public download URL.
   */
  uploadCentralCategoryIcon: async (categoryId, file) => {
    const extension = file.name.split('.').pop();
    const storageRef = ref(storage, `central/categories/${categoryId}/${Date.now()}.${extension}`);
    const snapshot = await uploadBytes(storageRef, file);
    return getDownloadURL(snapshot.ref);
  },

  /**
   * Upload a central product image (admin-created, not org-scoped).
   * Returns the public download URL.
   */
  uploadCentralProductImage: async (productId, file) => {
    const extension = file.name.split('.').pop();
    const storageRef = ref(storage, `central/products/${productId}/${Date.now()}.${extension}`);
    const snapshot = await uploadBytes(storageRef, file);
    return getDownloadURL(snapshot.ref);
  },
  ...
  uploadGroupImage: async (orgId, groupId, file) => {
    const extension = file.name.split('.').pop();
    const storageRef = ref(
      storage,
      `organizations/${orgId}/groups/${groupId}/${Date.now()}.${extension}`
    );
    const snapshot = await uploadBytes(storageRef, file);
    return getDownloadURL(snapshot.ref);
  },

  uploadTagImage: async (orgId, tagId, file) => {
    const extension = file.name.split('.').pop();
    const storageRef = ref(
      storage,
      `organizations/${orgId}/tags/${tagId}/${Date.now()}.${extension}`
    );
    const snapshot = await uploadBytes(storageRef, file);
    return getDownloadURL(snapshot.ref);
  },
...
// ---------------------------------------------------------------------------
// CentralCategoryFactory — centrale categoriebibliotheek (root level)
// ---------------------------------------------------------------------------
export const CentralCategoryFactory = {
  collection: () => collection(db, 'categories'),
  doc: (categoryId) => doc(db, 'categories', categoryId),
  ...
  create: async ({ name, iconUrl = '', color = '#4CAF50', approvedBy, sourceOrgId = null, sourceCategoryId = null }) =>
  ...
  update: (categoryId, data) => {
    const payload = { ...data };
    if (data.name) payload.nameLower = data.name.toLowerCase().trim();
    return updateDoc(doc(db, 'categories', categoryId), payload);
  },

  delete: (categoryId) => deleteDoc(doc(db, 'categories', categoryId)),
};
...
export const CentralProductFactory = {
  ...
  create: async ({ name, imageUrl, unit, approvedBy, sourceOrgId = null, sourceProductId = null, centralCategoryId = null }) =>
...
export const ProductSubmissionFactory = {
  ...
  create: async ({ name, imageUrl, unit, orgId, orgProductId, orgCategoryId = null, orgCategoryName = null, orgCategoryIconUrl = null, orgCategoryColor = null, orgCategoryCentralId = null }) =>
...
export const AccessRequestFactory = {
  collection: () => collection(db, 'accessRequests'),

  doc: (requestId) => doc(db, 'accessRequests', requestId),

  create: ({ requestingUserId, requestingUserEmail, requestingUserName, targetOrgId, targetOrgName }) =>
  ...
  updateStatus: (requestId, status, processedBy) =>
