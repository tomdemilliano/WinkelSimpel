...
export async function getCurrentUserClaims() {
  ...
  return {
    ...
    orgType: idTokenResult.claims.orgType || 'organization',
  };
}
...
export function useUserClaims() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        // Eerste poging met gecachte token
        let idTokenResult = await firebaseUser.getIdTokenResult();
        // Als role ontbreekt: forceer token refresh (custom claims nog niet actief)
        ...
        // Token ingetrokken (bv. na revokeRefreshTokens bij rolwijziging)
        // Uitloggen en opnieuw laten inloggen
        console.warn('Token refresh failed, signing out:', err.code);
...
export function withRoleGuard(allowedRoles, PageComponent) {
  ...
  function GuardedPage(props) {
    ...
    // Gebruik ref voor router om te voorkomen dat router-object-instabiliteit
    // de useEffect oneindig triggert
    ...
    useEffect(() => {
      ...
      // Wacht als role nog null is (token refresh bezig)
    ...
    // router bewust NIET in deps — routerRef.current is altijd actueel
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [claims, loading]);

    if (loading || !claims || !claims.role || !roles.includes(claims.role)) {
      return (
