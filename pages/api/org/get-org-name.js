...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!caller.orgId) return res.status(200).json({ name: null });

    const orgDoc = await adminDb.collection('organizations').doc(caller.orgId).get();
    const name = orgDoc.exists ? (orgDoc.data().name || null) : null;

    return res.status(200).json({ name });
  } catch (err) {
    console.error('get-org-name error:', err);
